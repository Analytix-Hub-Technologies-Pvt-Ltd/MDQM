...
import { enterpriseStewardCatalogDetail } from "../../enterpriseApi";
...
export default function StewardCatalogDetailModal({ datasetId, open, onClose }) {
  ...
  const load = useCallback(async () => {
    ...
    setErr("");
    try {
      const res = await enterpriseStewardCatalogDetail(datasetId);
  ...
  const hasTableData =
    dataLoaded && tables.some((t) => (t.row_count || 0) > 0 || (t.sample_rows || []).length > 0);
        ...
        <div className="flex items-start gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Validation rules, data quality results, and smart charts (steward view)
            </p>
            <div className="mt-2 flex flex-wrap gap-1.5">
              <StatusBadge status={cat.certification} />
              {cat.tier ? <StatusBadge status={cat.tier} /> : null}
            </div>
          </div>
        </div>
      }
      ...
      {loading ? (
        ...
        <div className="grid gap-4 lg:grid-cols-12 lg:items-start">
          ...
          <div className="lg:col-span-5 xl:col-span-4 lg:sticky lg:top-0">
            <DatasetCatalogChartInsights datasetId={datasetId} enabled={hasTableData} audience="steward" />
          </div>
        </div>
      )}
    </AppModal>
